...
**职责：** 基于 META4 对象级涡旋标注构建像素级训练标签，训练 U-Net 做涡旋分割与识别，并输出边界、中心与旋转方向。同时提供**真实物理演变追踪**能力，支持对单体涡旋生命周期的高保真追溯。

**真实物理追踪算法（涡旋时空演变追踪）：**
- **实时前后向推理：** 在追踪请求时，后端直接调用深度学习模型，基于原始动力地形与流速数据（ADT、UGOS、VGOS）动态执行目标日期前后最多 15 天（共计 31 天）的实时掩膜推断，单次计算耗时低至秒级。
- **空间近邻与形态匹配：** 以选中涡旋为锚点向前后延展，利用空间距离阈值（最大容忍每日 15 像素物理漂移）与相同极性判定，执行基于欧式距离的最近邻时空连线。
- **断点续传容忍度（Patience）：** 引入容忍度机制（Patience = 2 天）。当涡旋信号在某些日期因模型置信度波动出现“闪烁”或短暂丢失时，允许轨迹“隐身”悬空，并在后续时间重新匹配接续，大幅提升全生命周期轨迹的鲁棒性。
- **真实强度采样：** 轨迹上每一天的涡旋强度（核心 ADT 异常值）、影响面积、等效半径与物理坐标均严格对应模型在该日的实际演算输出，彻底摒弃伪造的抛物线模拟，真实还原海洋中尺度涡旋“生成-发展-成熟-消散”甚至突然碎裂的不稳定物理过程。
